...
async function updatePodcastMd(filePath, episodesData) {
  try {
    ...
    newFrontmatter += 'layout: page\n'; // You might want to change this
    newFrontmatter += 'permalink: /pods/\n'; // and this

    newFrontmatter += 'episodes:\n';
    episodesData.forEach(episode => {
      newFrontmatter += `  - data:\n`
      newFrontmatter += `     title: "${episode.title}"\n`;
      newFrontmatter += `     url: "docs/podcasts/${episode.slug}"\n`; // Adjust the path
    });
    newFrontmatter += '---\n';
...
const outputDirectory = './docs/podcasts';
const podcastMdFilePath = './pods.md'; // Path to your podcast.md
